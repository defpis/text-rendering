...
import textURL from "~/assets/print-cjk.txt";
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(textURL).then((resp) => resp.text()),
    ]).then(([font, text]) => {
      ...
      for (const glyph of font.stringToGlyphs(text)) {
        const offsetX = col * fontSize;
        const offsetY = row * fontSize + fontSize;
        ...
        triangles.forEach(({ p0, p1, p2 }) => {
          positionsAndColors.push(
            p0.x, p0.y, 1, 1, 0, ...(debug ? [0, 1, 0] : [0, 0, 0]),
            p1.x, p1.y, 1, 0, 1, ...(debug ? [0, 1, 0] : [0, 0, 0]),
            p2.x, p2.y, 1, 0, 0, ...(debug ? [0, 1, 0] : [0, 0, 0]),
          );
        });
