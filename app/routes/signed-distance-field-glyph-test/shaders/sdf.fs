...
void main() {
  ...
  for (int i = 0; i < int(v_count); i++) {
    Curve curve = load_curve(v_start + float(i));
    ...
    vec2 p = convert_space(inverse_matrix, v_uv);
    ...
    float a = 1.0 - clamp(scale * distance / u_max, 0.0, 1.0);

    r = max(r, a);
  }

  o_color = vec4(r * 0.5, 0.0, 0.0, 1.0);
}
