...
): CubicBezierSplitResult[] => {
  const result: CubicBezierSplitResult[] = [];
  cubics.forEach((cubic) =>
    isArray(cubic.cubics)
      ? result.push(...flattenCubic(cubic.cubics))
      : result.push(cubic),
  );
  return result;
};
...
export const pathToPolygons = (path: opentype.Path, sign: -1 | 1) => {
  ...
  const _innerCubics: CubicBezierSplitResult[] = [];
  const _outerCubics: CubicBezierSplitResult[] = [];
  ...
  const processCubic = (cubic: CubicBezierSplitResult) => {
    ...
    if (areaSigned * sign < 0) {
      cubic.points.push(cubic.p1);
      cubic.points.push(cubic.p2);

      _innerCubics.push(cubic);
    } else {
      _outerCubics.push(cubic);
    }
  ...
  let outers = _outerCubics;
  let inners = _innerCubics;
  ...
  while (loop < 3) {
    ...
    for (const outer of outers) {
      for (const inner of inners) {
        if (insideCubic(outer, inner.p1) || insideCubic(outer, inner.p2)) {
          const [c1, c2] = splitCubic(outer, 0.5);

          outer.points.push(c1.points, c1.p3, c2.points);
          outer.cubics = [c1, c2];

          const [c3, c4] = splitCubic(inner, 0.5);

          c3.points.push(c3.p1, c3.p2);
          c4.points.push(c4.p1, c4.p2);

          inner.points.splice(0, 2, c3.points, c3.p3, c4.points);
          inner.cubics = [c3, c4];
  ...
  const outerCubics = flattenCubic(_outerCubics);
  const innerCubics = flattenCubic(_innerCubics);
